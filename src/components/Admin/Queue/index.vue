    ...
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Danh sách yêu cầu cứu hộ </p>
            </div>
                    ...
                    <div class="row g-0">
                        <div class="col-auto">
                            <div class="bg-light d-flex align-items-center justify-content-center"
                                ...
                                <img v-if="request.imageUrl" :src="request.imageUrl" class="img-fluid h-100"
                        ...
                        <div class="col">
                            <div class="card-body d-flex flex-column h-100">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div>
                                        <span class="badge bg-secondary bg-opacity-10 text-secondary me-2">SOS-{{
                                            request.id }}</span>
                                        <span class="badge rounded-pill px-3 py-2" :class="request.statusClass">{{
                                            request.statusLabel }}</span>
                                    </div>
                                    <span class="badge bg-dark bg-opacity-10 text-dark">{{ request.priorityLabel
                                        }}</span>
                                </div>
                                ...
                                <div class="mb-3">
                                    <p class="mb-1 text-muted"><i class="bi bi-geo-alt-fill me-1"></i>{{ request.address
                                        }}</p>
                                    <p class="mb-1 text-muted"><i class="bi bi-people-fill me-1"></i> {{ request.people
                                        }} người</p>
                                    <p class="mb-1 text-muted"><i class="bi bi-card-text me-1"></i> {{
                                        request.description }}</p>
                                </div>

                                <div class="mt-auto d-flex flex-wrap gap-2">
                                    <button type="button" class="btn btn-sm d-flex align-items-center gap-2"
                                        :class="request.buttonClass" @click="changeRequestStatus(request)"
                                        :disabled="request.updating">
                                        <i class="bi bi-arrow-repeat"></i>
                                        {{ request.buttonLabel }}
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-danger" @click="cancelRequest(request)">
